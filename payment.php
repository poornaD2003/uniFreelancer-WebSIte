...
<style>
    .payment-container { padding: 140px 5% 80px; max-width: 1100px; margin: 0 auto; font-family: 'Outfit', sans-serif; }
    .payment-grid { display: grid; grid-template-columns: 1fr 1.3fr; gap: 2.5rem; }
    .summary-card, .payment-card { background: #ffffff; border: 1px solid rgba(16, 185, 129, 0.14); border-radius: 18px; padding: 2.5rem; box-shadow: 0 4px 24px rgba(16, 185, 129, 0.05); height: fit-content; }
    .summary-title { font-size: 1.4rem; font-weight: 700; margin-bottom: 1.5rem; color: #0f172a; }
    .summary-item { display: flex; justify-content: space-between; margin-bottom: 1rem; color: #64748b; font-size: 0.95rem; }
    .summary-item.total { border-top: 2px dashed rgba(16, 185, 129, 0.14); padding-top: 1.2rem; margin-top: 1.2rem; font-size: 1.3rem; font-weight: 800; color: #0f172a; }
    .payment-tabs { display: flex; gap: 1rem; margin-bottom: 2rem; }
    .pay-tab { flex: 1; text-align: center; padding: 1rem; border: 1px solid rgba(16, 185, 129, 0.14); border-radius: 12px; cursor: pointer; font-weight: 600; color: #64748b; background: #f8fafc; transition: all 0.3s ease; }
    .pay-tab.active { border-color: #10b981; color: #10b981; background: rgba(16, 185, 129, 0.05); }
    .pay-tab i { display: block; font-size: 1.5rem; margin-bottom: 0.5rem; }
    .form-section { display: none; }
    .form-section.active { display: block; animation: fadeIn 0.4s ease; }
    .input-group { margin-bottom: 1.2rem; }
    .input-group label { display: block; font-size: 0.8rem; font-weight: 700; color: #64748b; text-transform: uppercase; margin-bottom: 0.5rem; }
    .input-group input { width: 100%; padding: 1rem 1.2rem; border-radius: 10px; border: 1px solid #cbd5e1; background: #f8fafc; color: #0f172a; outline: none; font-size: 0.95rem; }
    .input-group input:focus { border-color: #10b981; background: #fff; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.15); }
    .btn-pay { width: 100%; padding: 1.2rem; border-radius: 12px; background: #10b981; color: white; font-size: 1.1rem; font-weight: 700; border: none; cursor: pointer; display: flex; justify-content: center; align-items: center; gap: 10px; box-shadow: 0 6px 20px rgba(16, 185, 129, 0.2); }
    .btn-pay:hover { background: #059669; }
    .loader { display: none; width: 20px; height: 20px; border: 3px solid rgba(255,255,255,0.3); border-radius: 50%; border-top-color: white; animation: spin 1s infinite linear; }
...
<script>
    function switchMethod(method) {
        ...
        if (isCard) {
            ...
            document.getElementById('btnText').innerText = "Pay Rs. <?php echo number_format($total_amount, 2); ?>";
        } else {
            ...
            document.getElementById('btnText').innerText = "Submit Bank Deposit";
        }
    }
    ...
    document.getElementById('paymentForm').addEventListener('submit', function() {
        ...
        document.getElementById('btnText').innerText = 'Processing secure transfer...';
