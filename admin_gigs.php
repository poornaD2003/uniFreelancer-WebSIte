<?php
// ─────────────────────────────────────────────────────────────────────────────
// admin_gigs.php — Gig Management Page
// Lists all gigs with search, status stat cards, and approve/suspend actions.
// ─────────────────────────────────────────────────────────────────────────────
include_once __DIR__ . '/includes/admin_common.php';

// ── HANDLE MODERATION ACTIONS (POST) ────────────────────────────────────────
// Triggered when the admin clicks an action button in the gig table
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['id'])) {
    ...
    // Flash result and redirect to prevent duplicate POST on browser refresh
    if ($ok) {
...
// ── SEARCH / FETCH GIGS ──────────────────────────────────────────────────────
// JOIN gigs with users to show the freelancer's name alongside each gig
$search = trim($_GET['search'] ?? '');
if ($search !== '') {
    // Filtered query: match gig titles that contain the search term
    ...
    $like = '%' . $search . '%';    // Wrap search term with % wildcards for LIKE
    ...
    // No search — return all gigs, newest first
...
// ── STAT CARD COUNTS ─────────────────────────────────────────────────────────
// Always query the full table so stats are accurate even when a search is active
$stats = [
    'total'    => count($all_gigs),
    'pending'  => admin_count_query($conn, "SELECT COUNT(*) AS total FROM gigs WHERE status = 'pending'"),
    'approved' => admin_count_query($conn, "SELECT COUNT(*) AS total FROM gigs WHERE status = 'approve'"),  // 'approve' not 'approved'
];

// ── RETRIEVE AND CLEAR FLASH MESSAGE ─────────────────────────────────────────
$flash = $_SESSION['admin_flash'] ?? null;
unset($_SESSION['admin_flash']); // Remove after reading so it only shows once

include 'includes/header.php';
?>

<?php
// Inject the 'gigs' colour theme CSS (pink palette) and shared admin layout
echo admin_theme_styles('gigs');
?>
...
<div class="admin-shell">
    ...
    <!-- Navigation pill bar — 'gigs' tab is highlighted as active -->
    ...
    <!-- ── STAT CARDS ──────────────────────────────────────────────────────── -->
    <!-- Three summary cards: Total | Approved (green) | Pending (orange) -->
    <div class="metric-grid">
        <div class="admin-panel metric-card">
            <div class="metric-label">Total Gigs</div>
            <div class="metric-value"><?php echo $stats['total']; ?></div>
            <div class="metric-note">All posted gigs</div>
        </div>
        <div class="admin-panel metric-card">
            <div class="metric-label">Approved</div>
            <div class="metric-value" style="color: #10b981;"><?php echo $stats['approved']; ?></div><!-- green = live/visible -->
            <div class="metric-note">Visible on the platform</div>
        </div>
        <div class="admin-panel metric-card">
            <div class="metric-label">Pending</div>
            <div class="metric-value" style="color: #f97316;"><?php echo $stats['pending']; ?></div><!-- orange = needs review -->
            <div class="metric-note">Waiting for review</div>
        </div>
    </div>
    ...
    <div class="section-card admin-panel">
        <div class="section-head" style="flex-wrap: wrap;">
            ...
            <form method="GET" style="display:flex; gap:0.5rem; align-items:center;">
                <input type="text" name="search" placeholder="Search gig..."
                ...
                <?php if (!empty($_GET['search'])): ?>
                    <!-- Clear button resets the search and shows all gigs -->
                    <a href="admin_gigs.php" class="btn"
        ...
        <?php if (empty($all_gigs)): ?>
            <!-- Empty state when no gigs match or none exist yet -->
                ...
                <tbody>
                    <?php foreach ($all_gigs as $gig): ?>
                        <tr>
                            ...
                            <!-- Freelancer name (joined from the users table) -->
                            <td><?php echo htmlspecialchars($gig['student_name']); ?></td>

                            <!-- Category pill — always blue (pill-info) -->
                            ...
                            <td><span class="pill <?php echo admin_status_class('gig', $gig['status']); ?>">
                                <?php echo htmlspecialchars(admin_status_label('gig', $gig['status'])); ?>
                            </span></td>

                            <!-- Price formatted as whole number with Rs. prefix -->
                            <td>Rs. <?php echo number_format((float)$gig['price'], 0); ?></td>

                            <!-- Submission date, no-wrap to keep it on one line -->
                            <td style="white-space: nowrap;"><?php echo date('M d, Y', strtotime($gig['created_at'])); ?></td>

                            <!-- Action buttons — only pending gigs show the Approve button -->
                            <td><div class="action-stack">
                                <?php if ($gig['status'] === 'pending'): ?>
                                    <!-- Blue approve button — makes the gig visible on the platform -->
